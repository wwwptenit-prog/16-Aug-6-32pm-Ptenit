    ...
    <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200/90 dark:border-slate-700/80 shadow-sm hover:shadow-2xl hover:border-[#1DB954] transition-all duration-300 flex flex-col overflow-hidden group">
        ...
        <div className="absolute top-2 left-2 sm:top-3 sm:left-3 flex items-center gap-1 sm:gap-2">
          {course.isFree ? (
            <span className="px-2 py-0.5 sm:px-3 sm:py-1 rounded-full bg-emerald-500 text-white font-bold text-[9px] sm:text-xs shadow-md uppercase tracking-wider flex items-center gap-0.5 sm:gap-1">
              <Tag className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> {t('ফ্রি', 'Free')}
            </span>
          ) : (
      ...
      <div className="p-2.5 sm:p-4.5 md:p-5 flex-1 flex flex-col justify-between space-y-2 sm:space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-1 pt-0.5 sm:pt-1">
          <div>
            {course.isFree ? (
              <span className="text-xs sm:text-lg font-black text-emerald-500">{t('ফ্রি!', 'Free!')}</span>
            ) : (
              <div className="flex items-baseline gap-1">
                <span className="text-xs sm:text-lg font-black text-slate-900 dark:text-white font-heading">
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={() => onOpenDetail(course.id)}
              className="px-2 py-1 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-bold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 transition-colors cursor-pointer"
            >
              {t('বিস্তারিত', 'Details')}
            </button>
            <button
              onClick={() => onQuickEnroll(course)}
              className="px-2 py-1 sm:px-3.5 sm:py-2 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-bold text-white bg-[#1DB954] hover:bg-emerald-500 shadow-xs sm:shadow-md sm:shadow-[#1DB954]/20 transition-all cursor-pointer flex items-center gap-0.5 sm:gap-1"
            >
              {t('এনরোল', 'Enroll')}
