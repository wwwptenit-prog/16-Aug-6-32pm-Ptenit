...
export const DigitalProductsSection: React.FC<DigitalProductsSectionProps> = ({ setActiveTab }) => {
  ...
  // =========================================================================
  // 🌟 DEDICATED FULL LANDING PAGE VIEW (Not a popup modal!)
  // =========================================================================
  if (selectedProduct) {
    const isFree = selectedProduct.price === 0;

    return (
      <div className="space-y-6 pt-6 border-t border-slate-200 dark:border-slate-800 font-bengali animate-fadeIn">
        {/* Top Sticky Navigation Bar */}
        <div className="bg-white dark:bg-slate-900/90 backdrop-blur-md p-3.5 sm:p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 shadow-xs">
          <button
            type="button"
          ...
          >
            ...
            <span>{t('← সব প্রোডাক্টে ফিরে যান', '← Back to All Products')}</span>
          </button>
    ...
    <div className="space-y-6 pt-8 border-t border-slate-200 dark:border-slate-800 font-bengali">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className="hidden sm:flex px-3.5 py-1.5 bg-emerald-500/10 text-[#1DB954] border border-[#1DB954]/20 rounded-full text-xs font-extrabold items-center gap-1.5 shadow-xs">
            <Mail className="w-3.5 h-3.5" />
            ফ্রি & প্রিমিয়াম ডাউনলোড
          </span>
        </div>
      </div>

      {/* Grid: 5 columns on PC, 2 columns on Phone (Max 4 on mobile unless expanded) */}
      <div>
        {/* Desktop: 1 row of 5 */}
        <div className="hidden sm:grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-5 gap-4 lg:gap-5">
          {digitalProducts.slice(0, 5).map(product => {
            ...
            return (
              ...
              >
                ...
                {/* Card Footer: Price on Left, Single "বিস্তারিত" Button on Right */}
                <div className="p-2.5 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    {isFree ? (
                      <span className="text-xs sm:text-sm font-black text-emerald-500 dark:text-emerald-400 block truncate">
                        ফ্রি
                      </span>
                    ) : (
                      <>
                        {product.originalPrice && (
                          <span className="text-[10px] text-slate-400 line-through block leading-none truncate">
                            ৳{product.originalPrice.toLocaleString('bn-BD')}
                          </span>
                        )}
                        <span className="text-xs sm:text-sm font-black text-[#1DB954] block truncate">
                  ...
                  <button
                    type="button"
                    onClick={() => handleOpenDetail(product)}
                    className="px-3 py-1.5 rounded-xl bg-[#1DB954] hover:bg-emerald-600 text-white font-extrabold text-[11px] sm:text-xs flex items-center gap-1 shadow-xs transition cursor-pointer active:scale-95 shrink-0"
                  >
                    <span>বিস্তারিত</span>
            ...
            return (
              ...
              >
                ...
                <div className="p-2 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-1.5">
                  <div className="min-w-0">
                    <span className={`text-xs font-black block truncate ${isFree ? 'text-emerald-500' : 'text-[#1DB954]'}`}>
                      {isFree ? 'ফ্রি' : `৳${product.price.toLocaleString('bn-BD')}`}
                    </span>
                  </div>
                  ...
                  <button
                    type="button"
                    onClick={() => handleOpenDetail(product)}
                    className="px-2 py-1 rounded-lg bg-[#1DB954] hover:bg-emerald-600 text-white font-extrabold text-[10px] flex items-center gap-0.5 shadow-xs transition cursor-pointer active:scale-95 shrink-0"
                  >
                    <span>বিস্তারিত</span>
